...
resource "azurerm_federated_identity_credential" "pg_backup_fed" {
  ...
  subject             = "system:serviceaccount:database-${local.environment}:tim-db" # Match your DB name/ns
}
...
resource "kubernetes_config_map_v1" "infra_outputs" {
  metadata {
    name      = "infra-outputs"
    namespace = kubernetes_namespace_v1.db_${local.environment}.metadata[0].name
  }
  data = {
